...
<link type="text/css" rel="stylesheet" charset="utf-8" href="./modules/upload/theme/default/list.css">
...
<div id="uplist">
	<div id="captionDiv" class="caption">
		...
		<table>
		<tr>
		<td class="x1">파일명 <input type="text" name="name" value="" class="input" /> 링크</td>
	...
	<li id="uLi<?php echo $U['uid']?>">
		...
		<a class="hand" onmousemove="upfile_thumb('<?php echo getExt($U['name'])?>','<?php echo $U['url'].$U['folder'].'/'.$U['thumbname']?>');" onclick="captionCheck('<?php echo $U['uid']?>');"<?php if($U['type']==2):?> ondblclick="dropEditArea('<?php echo $U['url'].$U['folder'].'/'.$U['tmpname']?>','<?php echo $U['width']?>');" title="*한번클릭:파일명/링크수정"<?php endif?>><?php echo $U['name']?></a>
...
//	else {
	...
	if (confirm(tmsg))
	{
		...
		getId('uLi'+uid).innerHTML = "";
		getId('thumbbox').style.background = '';
		frames._action_frame_<?php echo $m?>.location.href = '<?php echo $g['s']?>/?r=<?php echo $r?>&m=upload&a=files_delete&ptr=1&file_uid=' + uid;
