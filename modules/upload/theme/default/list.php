...
<div id="uplist">
	...
	<li id="uLi<?php echo $U['uid']?>">
		<input type="checkbox" id="uCk<?php echo $U['uid']?>" name="filemembers[]" value="<?php echo $U['uid']?>" checked="checked" />
...
function dropEditArea(pic,w)
{
	...
	if (parent.getId('<?php echo $gparamExp[1]?>'))
	{
		if (confirm('이미 본문에 삽입된 이미지일 수 있습니다.\n정말로 본문에 삽입하시겠습니까?    '))
		{
			...
			parent.SetEditor('<?php echo $gparamExp[1]?>',photos);
		}
	}
	<?php endif?>
}
function upfile_delete(uid,type,tmpname)
{
	...
	if (confirm(tmsg))
	{
		if (parent.getId('<?php echo $gparamExp[1]?>'))
		{
			var content = parent.GetEditor("<?php echo $gparamExp[1]?>");
		...
		if (parent.getId('<?php echo $gparamExp[1]?>'))
		{
			parent.ClearEditor('<?php echo $gparamExp[1]?>');
			parent.SetEditor('<?php echo $gparamExp[1]?>',resultc);
		}
