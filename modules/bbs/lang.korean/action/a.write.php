...
include_once $g['dir_module'].'var/var.'.$B['id'].'.push.php';

if($itms){
	for($i=0;$i<count($itms);$i++)
		$adddata .= $itms[$i];
	$adddata = substr($adddata,1);
}
...
$name		= $my['uid'] && $my['level'] < $admin['admin'] ? $my['name'] : trim($name);
$nic		= $my['uid'] && $my['level'] < $admin['admin'] ? $my['nic'] : $name;
...
else 
{
	...
	if($d['bbs']['email']){
		$_NOW			= getDbData($table[$m.'data'],"site='$s' and gid='$gid' and bbs='$bbsuid' and bbsid='$bbsid'","*");
		$_ADM			= getDbData("{$table['s_mbrid']} a, {$table['s_mbrdata']} b","a.uid = b.memberuid and a.id = 'admin'","*");
		$_ADM['email'] 	= $_ADM['email'] ? $_ADM['email'] :'admin@'.$_SERVER['HTTP_HOST'];
		$to			= $d['bbs']['email'];
		$from		= $_ADM['email'].'|'.$_HS['name'];
		$sbj	= '['.$B['name'].'] '.$subject;
		...
		foreach($tmp as $_T){
			$itm = explode(":",$_T);
			$strTmp .= "<div>{$itm[0]} : {$itm[1]}</div>";
		}
		
		$tmp		= str_replace("\\","",$content);
		$ml_content	= "
			<div style='font-size:1.2em;font-weight:bold;'>{$subject} [<a href='{$g['url_root']}/?r={$r}&c={$c}&mod=view&uid={$_NOW['uid']}'>게시물 바로가기</a>]</div>
			<div>작성자 : {$name}</div>
			<div>제목 : {$subject}</div>
			{$strTmp}
			<div style='margin-top:25px;'>{$tmp}</div>
		";

		include_once $g['path_core'].'function/email.func.php';
		getSendMail($to,$from,$sbj,$ml_content,'HTML');
	}
