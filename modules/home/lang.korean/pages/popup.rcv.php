...
<div id="paperbox">
	...
	<tr>
		<td>
			<div>보낸사람 : <? if($M['memberuid']==$my['uid']) { ?>나<? }else{ ?><?=$M['nic']?>님<? } ?></div>
			<div>보낸시간 : <span class="dt"><?=getDateFormat($R['d_regis'],'Y-m-d H:i')?></span></div>
			<div class="msg"><?=getContents($R['content'],$R['html'],'')?></div>
			<div class="pagebox01">
	...
	<tr>
		<td class="submit">
			...
			<?php if($R['by_mbruid']):?>
			<input type="button" value=" 답장 " class="btnblue b" onclick="parent.getLayerBox('<?=$g['s']?>/?r=<?=$r?>&system=popup.papersend&iframe=Y&rcvmbr=<?=$R['by_mbruid']?>','메세지 보내기',300,270,'',false,'');" />
			<?php endif?>
			<?php if($R['inbox']==1):?>
