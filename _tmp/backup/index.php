...
$g = array(
	...
	'path_page'   => './pages/',
	'path_file'   => './files/',
	'path_image'  => './files/_etc/images/',
	'sys_lang'    => 'korean',
	'window_full' => 'min'			// 전체 화면 사용여부 (min:작은사이즈)
);
...
if (is_file($g['path_var'].'db.info.php'))
{
	...
	require $g['path_var'].'admin.info.php';
...
if ($_SESSION['appmode'] && $g['mobile'])	$iframe = 'Y';

if ($m=='admin' || $iframe=='Y' || $ajax=='Y') $d['layout']['php'] = $_HM['layout'] = '_blank/main.php';
else {
	...
	if ($g['mobile'] && $_SESSION['appmode']=='Y') $d['layout']['php'] = $_HM['layout'] = 'mobile/app_main.php';
}
...
<?php endif?>
<link href="<?=$g['s']?>/files/_etc/images/favicon.ico" rel="icon" type="image/x-icon" />
