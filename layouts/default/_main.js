...
// ==================================================
// 글로벌 오브젝트
var loginbar_banner	= null;
var topmenu			= null;
var quickmenu		= null;

// 상단메뉴
topmenu = function() {
	var offSet = $('#topmenu').offset().top; + $('#topmenu').height();
	onInit();
	
	// member function
	this.allMenu = function() {
		$('#topmenu').find('.all-box').slideToggle();
	}
	...
	function setPosition() {
		var top = offSet;
		if($('#loginbar_banner').is(':hidden')) {
			top = offSet - $('#loginbar_banner').height();
		}

		if (top < $(window).scrollTop()) {
			$('#topmenu').attr('style','position:fixed; top:0;');
		}
		else {
			$('#topmenu').attr('style','');
		}
...
quickMenu = function(id) {
	...
	function setPosition() {
		var left = (($(window).width()-$(".wrap").width())/2) + $(".wrap").width() + 10;
		$(id).attr("style","left:"+left+"px;");
		$(id).show();
	}
